...
import com.codenvy.api.vfs.server.impl.memory.context.MemoryItem;
import com.codenvy.api.vfs.server.impl.memory.context.MemoryItemVisitor;
...
import com.codenvy.api.vfs.shared.ItemNodeImpl;
...
import static com.codenvy.api.vfs.server.observation.ChangeEvent.ChangeType;
...
public class MemoryFileSystem implements VirtualFileSystem {
    ...
    public void delete(@PathParam("id") String id, //
        ...
        if (object.isFile()) {
            ...
            final List<MemoryItem> toDelete = new ArrayList<>();
    ...
    public ItemList getChildren(@PathParam("id") String folderId, //
        ...
        ItemList il = new ItemListImpl(l);
    ...
    private List<ItemNode> getTreeLevel(MemoryItem object, int depth, boolean includePermissions, PropertyFilter propertyFilter)
        ...
        MemoryFolder folder = (MemoryFolder)object;
        List<MemoryItem> children = folder.getChildren();
        List<ItemNode> level = new ArrayList<>();
    ...
    public ItemList getVersions(@PathParam("id") String id, //
        ...
        if (maxItems < 0 || (maxItems - skipCount) > 0) {
            l = new ItemListImpl(Collections.singletonList(fromMemoryItem(object, false, propertyFilter)));
            l.setHasMoreItems(false);
        } else {
            l = new ItemListImpl(Collections.<Item>emptyList());
            l.setHasMoreItems(false);
        }
        return l;
    }
    ...
    @Path("lock/{id}")
    @Override
    public Lock lock(@PathParam("id") String id, @QueryParam("timeout") @DefaultValue("0") long timeout)
        ...
        return new LockImpl(context.getCurrentVirtualFileSystemUser().getUserId(), ((MemoryFile)object).lock(timeout), timeout);
    }
    ...
    public Item move(@PathParam("id") String id, //
        ...
        if (object.isFolder()) {
            ...
            final List<MemoryItem> forMove = new ArrayList<>();
            object.accept(new MemoryItemVisitor() {
                @Override
                public void visit(MemoryItem i) throws VirtualFileSystemException {
                    if (i.isFolder()) {
    ...
    public ContentStream exportZip(@PathParam("folderId") String folderId) throws IOException, VirtualFileSystemException {
        ...
        try {
            ...
            LinkedList<MemoryFolder> q = new LinkedList<>();
            q.add(exportFolder);
    ...
    private Item fromMemoryItem(MemoryItem object, boolean includePermissions, PropertyFilter propertyFilter, boolean addLinks)
        ...
        if (object.isFile()) {
            ...
            item = new FileImpl(vfsId, id, name, path, parentId, created, modified, versionId, mediaType, length,
                                ...
                                addLinks ? LinksHelper
                                        .createFileLinks(baseUri,
                                                         EnvironmentContext.getCurrent().getVariable(EnvironmentContext.WORKSPACE_NAME)
                                                                           .toString(), id, latestVersionId, path, mediaType, locked,
                                                         parentId) : null);
        } else {
            MemoryFolder folder = (MemoryFolder)object;
            if (folder.isProject()) {
                ...
                item = new ProjectImpl(vfsId, id, name, mediaType, path, parentId, created, folder.getProperties(propertyFilter),
                                       addLinks ? LinksHelper.createProjectLinks(baseUri, EnvironmentContext.getCurrent().getVariable(
                                               EnvironmentContext.WORKSPACE_NAME).toString(), id, parentId) : null, projectType);
            } else {
                ...
                item = new FolderImpl(vfsId, id, name, mediaType == null ? Folder.FOLDER_MIME_TYPE : mediaType, path, parentId, created,
                                      ...
                                      addLinks ? LinksHelper.createFolderLinks(baseUri, EnvironmentContext.getCurrent().getVariable(
                                              EnvironmentContext.WORKSPACE_NAME).toString(), id, isRoot, parentId) : null);
            }
        }
        ...
        if (includePermissions) {
            ...
            if (item.getPermissions() == null) {
                item.setPermissions(new HashSet<>(Arrays.asList(BasicPermissions.ALL.value())));
            }
        }
